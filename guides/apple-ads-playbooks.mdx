...
asc ads platform apps search \
  --ad-account "987654" \
  --query "Example" \
  --output json
```

The legacy Campaign Management API v5 commands under `asc ads` still run in
...
commands. The v5 `asc ads reports preset` helper remains a warning-producing
...
For raw calls, keep `asc ads api request` for v5 paths and its `--org` context.
It remains runnable with a warning and is not rewritten. Use
`asc ads platform api request --path v1/... --ad-account ...` for a raw v1 call.
